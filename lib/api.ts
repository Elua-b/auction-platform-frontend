...
export const productAPI = {
  getAll: (filters?: { categoryId?: string; sellerId?: string; status?: string }) => {
...
export const auctionAPI = {
  ...
  create: (data: any) => apiCall('/auctions', { method: 'POST', body: JSON.stringify(data) }),
  updateStatus: (id: string, status: string) =>
    apiCall(`/auctions/${id}/status`, { method: 'PATCH', body: JSON.stringify({ status }) }),
  delete: (id: string) => apiCall(`/auctions/${id}`, { method: 'DELETE' }),
}

// Bids
export const bidAPI = {
  place: (data: any) => apiCall('/bids', { method: 'POST', body: JSON.stringify(data) }),
  getByAuction: (auctionId: string) => apiCall(`/bids/auction/${auctionId}`),
  getByUser: (userId: string) => apiCall(`/bids/user/${userId}`),
  getByEventProduct: (eventProductId: string) => apiCall(`/bids/event-product/${eventProductId}`),
}
...
export const eventAPI = {
  ...
  updateStatus: (id: string, status: string) =>
    apiCall(`/events/${id}/status`, { method: 'PATCH', body: JSON.stringify({ status }) }),
...
export const categoryAPI = {
  ...
  update: (id: string, data: any) => apiCall(`/categories/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
